...
for d in target_dirs:
    try:
        ...
        f_md = os.path.join(d, 'Sensebar_AI_Agent_Videos_Full_Version.md')
        f_txt = os.path.join(d, 'sensebar_urls_for_notebooklm.txt')
        with open(f_md, 'w', encoding='utf-8') as f:
            f.write(md_content)
        with open(f_txt, 'w', encoding='utf-8') as f:
            f.write(txt_content)
        print("Successfully synced to:", d)
    except Exception as e:
        print("Error syncing to:", d, e)

print(f"COMPLETE! {len(all_urls)} videos written across all paths.")
