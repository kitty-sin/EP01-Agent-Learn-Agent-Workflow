...
print(f"Total fetched videos from channel: {len(videos)}")
...
print(f"Total videos published in 2025-2026: {len(videos_2025_plus)}")
...
md_lines.append(f"> **篩選與分類標準**：  \n> 1. **發佈時間限制**：僅收錄 **2025 年至 2026 年** 最新發布之影片（已剔除 2024 年及更早之舊影片）。  \n> 2. **嚴謹去重**：全面比對影片 ID 與標題，確保無任何重複影片。  \n> 3. **排除特定主題**：已徹底排除 Felo 相關影片。  \n> 4. **精準分類**：Claude AI/Code、GPT Codex、NotebookLM、AI 簡報、ChatGPT/OpenAI、AI 工具基本功 等主題均獨立分類，絕不混淆。  \n> 5. **換行格式**：每部影片網址均獨立換行呈現，方便直接貼入 NotebookLM。")
md_lines.append("")
md_lines.append("---")
md_lines.append("")
...
# Target absolute paths in workspace
target_dir = r"C:\Users\PC\Documents\Google%20Antigravity\2026-AI-Agent\Al_AgentEP01_用Agent來學習Agent"
if not os.path.exists(target_dir):
    target_dir = os.getcwd()

md_file_path = os.path.join(target_dir, "Sensebar_AI_Agent_Videos_Full_Version.md")
txt_file_path = os.path.join(target_dir, "sensebar_urls_for_notebooklm.txt")
...
print(f"DONE! Written {len(all_urls)} videos to {md_file_path}")
