...
# Rule 1: Strict Deduplication by normalized title (removing EP numbers/punctuations)
seen_keys = set()
unique_videos = []

for v in v_no_felo:
    title = v['title'].strip()
    # Normalize title key for deduplication
    clean_key = re.sub(r'[^\w\u4e00-\u9fff]', '', title.lower())
...
print(f"Total strictly unique videos: {len(unique_videos)}")

# Rule 2 & 4: Categorize strictly into distinct buckets
categories = {
...
print(f"Total strictly categorized 2025-2026 AI videos: {total_categorized}")
